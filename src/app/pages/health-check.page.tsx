...
import { STLIB, useOnMounted } from '../../lib';
...
export default function HealthCheckPage() {
  ...
  const checkHealth = async () => {
    try {
      ...
      setStatus(`Response: OK<->${STLIB.OK}`); // Update status directly
      console.log('Health check successful', status); // Debug log
    } catch (error: unknown) {
      if (error instanceof Error) {
        console.error(`Health check error: ${error.message}`);
        setStatus(`Response: Error<->${STLIB.INTERNAL_SERVER_ERROR}: ${error.message}`);
      }

      const errorMsg = String(error);
      console.error(`Health check error: ${errorMsg}`);
      setStatus(`Response: Error<->${STLIB.INTERNAL_SERVER_ERROR}: ${errorMsg}`);
    }
  };
